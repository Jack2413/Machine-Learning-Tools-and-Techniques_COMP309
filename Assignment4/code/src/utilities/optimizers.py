...
def gradient_descent(y, x, theta, max_iters, alpha, metric_type):
	...
	# for x,y in zip(x[x:mini_batch_size],y[0:mini_batch_size]):
...
def mini_batch_gradient_descent(y, x, theta, max_iters, alpha, metric_type, mini_batch_size):
    ...
    for j in range(max_iters):
        ...
        for i in range(0,num_of_samples,mini_batch_size):
            x_i = x[i:i+mini_batch_size]
            y_i = y[i:i+mini_batch_size]

            gradient = -2 * x_i.T.dot(y_i - x_i.dot(theta)) / num_of_samples
            theta = theta - alpha * gradient
            loss = compute_loss(y_i, x_i, theta, metric_type)
        ...
        print("MINIBGD({bi}/{ti}): loss={l}, w={w}, b={b}".format(
            bi = j, ti = max_iters - 1, l = loss, w = theta[0], b = theta[1]))
    return thetas, losses
